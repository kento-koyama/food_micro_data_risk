import streamlit as st
import pandas as pd
from pathlib import Path
import unicodedata

st.set_page_config(page_title="Contamination Data Translator", layout="centered")

st.title("食中毒汚染データ 英語変換アプリ")
st.markdown("CSVファイルをアップロードすると英語版に変換されたCSVをダウンロードできます。")

# 表記揺れ正規化関数（※カタカナ変換なし）
def normalize_text(text):
    if pd.isna(text):
        return text
    text = str(text)
    text = unicodedata.normalize("NFKC", text)  # 全角・半角の統一のみ
    return text


uploaded_file = st.file_uploader(
    "日本語のCSVファイルをアップロード（汚染率または汚染濃度）",
    type="csv"
)

if uploaded_file:
    df = pd.read_csv(uploaded_file, encoding="utf-8-sig")

    # 翻訳辞書ファイル
    translation_path = Path("translation_jpn_to_eng.csv")

    if not translation_path.exists():
        st.error(f"翻訳辞書ファイルが見つかりません: {translation_path}")
    else:
        translation_df = pd.read_csv(translation_path, encoding="utf-8-sig")

        # 辞書のクリーニング
        translation_df["Japanese"] = (
            translation_df["Japanese"].astype(str).str.strip()
        )
        translation_df["English"] = (
            translation_df["English"].astype(str).str.strip()
        )

        # 正規化（NFKCのみ）
        translation_df = translation_df.applymap(normalize_text)

        # DataFrame のカラム名クリーニング
        df.columns = df.columns.str.strip()

        translation_dict = dict(
            zip(translation_df["Japanese"], translation_df["English"])
        )

        # カラム名の翻訳
        df.columns = [translation_dict.get(col, col) for col in df.columns]

        # 値の正規化と翻訳
        df = df.applymap(normalize_text)
        df = df.replace(translation_dict)

        # 出力ファイル名
        output_name = (
            "contamination_rate.csv"
            if "率" in uploaded_file.name
            else "concentration_of_contamination.csv"
        )

        # ダウンロード
        st.success("変換が完了しました。")
        st.download_button(
            label="英語版CSVをダウンロード",
            data=df.to_csv(index=False).encode("utf-8-sig"),
            file_name=output_name,
            mime="text/csv",
        )
