...
st.sidebar.markdown("---")
st.sidebar.markdown(
    '<a href="https://github.com/kento-koyama/food_micro_data_risk" '
...
st.sidebar.markdown(
    '<a href="https://contamination-rate1-624097414875.asia-northeast1.run.app/" '
    'target="_blank" rel="noopener noreferrer" '
    'style="display:inline-flex; align-items:center; gap:0.4em; text-decoration:none;">'
    '<svg width="16" height="16" fill="currentColor" aria-hidden="true" '
    'xmlns="http://www.w3.org/2000/svg">'
    '<path d="M15.985 8.5H8.207l-5.5 5.5a8 8 0 0 0 13.277-5.5z"></path>'
...
# データがない場合は処理を中止して警告を表示
    ...
    if selected_bacteria in ["", "すべて"]:  # 細菌名の絞り込みがない場合に表示
        ...
        with col2:
            ...
            ax1.set_xlabel('検体数', fontsize=size_label+4)
            ax1.set_ylabel('細菌名', fontsize=size_label+4)
            ax1.set_title(f'細菌ごとの食品検体数\n{group_title}', fontsize=size_title+4)
            ax1.tick_params(axis='both', which='major', labelsize=size_label+4)
        ...
        df_Cperfringens_counts = df_filtered[df_filtered['細菌名'].str.contains('Clostridium perfringens')]
