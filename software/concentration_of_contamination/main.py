...
# データがない場合は処理を中止して警告を表示
    ...
    if selected_bacteria == "すべて":  # 細菌名の絞り込みがない場合に表示
        ...
        with col2:
            ...
            ax1.set_xlabel('検体数', fontsize=size_label+2)
            ax1.set_ylabel('細菌名', fontsize=size_label+2)
            ax1.set_title(f'細菌ごとの食品検体数\n{group_title}', fontsize=size_title+2)
            ax1.tick_params(axis='both', which='major', labelsize=size_label+2)
